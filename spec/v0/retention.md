...
The `MAINTENANCE` service owns retry expiration, idempotency-reservation expiration, data expiration, object reclamation, and provenance pruning. Retry expiration, idempotency-reservation expiration, data expiration, and provenance pruning MUST each start at least once per `retention.scan_interval_seconds` while the service is ready; object reclamation follows `garbage_collection.scan_interval_seconds`. No process may overlap two invocations of the same local retention task. No task requires a leader or instance affinity. Concurrent workers MUST remain correct through row locks, `SKIP LOCKED`, durable states, and reference revalidation.
