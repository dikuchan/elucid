//! Durable local admission spool.

mod checkpoint;
mod error;
mod frame;
mod model;
mod normalization;
mod output_recovery;
mod parquet;
mod recovery;
mod segment;
mod spool;

pub use elucid_core::EventId;
pub use elucid_storage::BatchId;
pub use error::{SpoolError, SpoolErrorKind, SpoolModelError};
pub use model::{
    AppendBodyLimit, BatchByteSize, BatchMetadata, BodyDigest, DurableAppend, IngestionTime,
    MaximumBatchAdmission, PinnedCatalogIdentities, RecoveredBatch, RecoveryReport,
    SpoolBatchRange, SpoolCapacity, SpoolCheckpoint, SpoolReclamation, SpoolUsage,
};
pub use normalization::{
    AcceptedRow, DEAD_LETTER_PAYLOAD_PREFIX_BYTES, DeadLetterCode, DeadLetterEntry,
    DeadLetterPayload, EventTime, JsonObject, MAXIMUM_BATCH_EVENT_DAYS, NormalizationError,
    NormalizedBatch, NormalizedField, NormalizedRecord, NormalizedValue, PayloadEncoding,
    PayloadExtent, RecordLocation, RecordPayloadDigest, normalize_records,
};
pub use output_recovery::{
    BatchOutputRequirements, BatchPositionCoverage, CheckpointPlan, OutputRecoveryAction,
    OutputRecoveryError, OutputRecoveryId, OutputRecoveryLog, OutputRecoveryModelError,
    OutputRecoveryObservation, OutputRecoveryReclamation, OutputRecoveryRecord,
    OutputRecoveryRecordOutcome, PlannedOutputBytes, PublishedOutput, RecoveryOutput,
    RetainedSpoolBytes, UnregisteredOutputBytes, plan_checkpoint,
};
pub use parquet::{SegmentMaterializationError, materialize_segment_record_batch};
pub use recovery::{RecoveredBatches, SpoolRecovery};
pub use segment::{
    EventDay, SealedSegment, SealingReason, SegmentBuildError, SegmentBuildOutcome,
    SegmentBuildSummary, SegmentBuilderModelError, SegmentBuilderUsage, SegmentBuilders,
    SegmentCapacity, SegmentKey, SegmentRow, SegmentStagingCapacity, SegmentTimeBounds,
};
pub use spool::{Spool, SpoolReservation};
